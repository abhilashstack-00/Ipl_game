...
export default function MatchesPage() {
  const { user } = useAuth()
  const { session, matches, fetchMatches, processMatch, chooseMatchTeam, addNotification } = useGame()
  ...
  const filtered = (matches || []).filter((match) => {
    if (filter === 'contest') return match.isContest || (match.bothSameUser && match.matchChoice)
    if (filter === 'mine') return myTeams.includes(match.team1) || myTeams.includes(match.team2)
    return true
  })
  ...
  const handleChooseTeam = async (match, chosenTeamId) => {
    setChoosing(match.id)
    try {
      await chooseMatchTeam(session.session.id, match.id, chosenTeamId)
      addNotification(`⭐ You chose ${getTeam(chosenTeamId).short}. The other team will count as the opponent for this match.`, 'success')
      setSelectedMatch(null)
    } catch (err) {
      addNotification(err.response?.data?.error || 'Failed to choose team', 'danger')
    } finally {
      setChoosing('')
    }
  }
      ...
      <div className="max-w-5xl mx-auto">
        <div className="flex items-center justify-between flex-wrap gap-3 mb-6">
          <div>
            ...
            <p className="text-gray-400 text-sm mt-1">
              {(matches || []).filter((m) => (m.isContest || (m.bothSameUser && m.matchChoice)) && !m.processed).length} active contests · {(matches || []).filter((m) => m.processed || m.result).length} completed
            </p>
          </div>
          ...
          <div className="flex gap-1 bg-white/5 p-1 rounded-xl">
            {[[ 'all', 'All' ], [ 'contest', '⚔️ Contests' ], [ 'mine', '🏏 My Teams' ]].map(([val, label]) => (
              <button
                ...
                className={`px-3 py-1.5 rounded-lg text-xs font-bold uppercase tracking-wider transition-all ${filter === val ? 'bg-yellow-400 text-ipl-deep-blue' : 'text-gray-400 hover:text-white'}`}
              >
                {label}
              </button>
            ))}
        ...
        <div className="space-y-2">
          {filtered.map((match) => {
            ...
            const hasChoice = !!match.matchChoice?.chosenTeamId
            const contestLike = isContestMatch || (bothSame && hasChoice)
            ...
            return (
              <div
                key={match.id}
                className={`rounded-xl p-4 border transition-all ${contestLike && !done ? 'border-orange-400/40 hover:border-orange-400/70' : done ? 'border-green-400/20 opacity-80' : 'border-white/10 hover:border-white/20'}`}
                style={{ background: contestLike && !done ? 'rgba(255,107,53,0.05)' : 'rgba(255,255,255,0.03)' }}
              >
                <div className="flex items-center gap-3 flex-wrap">
                  ...
                  <div className="flex-1 text-center min-w-0">
                    <div className="font-teko text-gray-400 text-lg">
                      ...
                      {done && match.result?.winner ? (
                        <span className="mx-2 text-white text-sm">{match.result.winner === match.team1 ? '🏆' : ''}</span>
                      ) : isSimulated ? (
                        <span className="mx-2 text-orange-400 text-sm font-bold">{simulResult.winner === match.team1 ? '★' : ''}</span>
                      ) : null}
                      {' '}vs{' '}
                    ...
                    <div className="flex justify-center flex-wrap gap-1.5 mt-1.5">
                      ...
                      {hasChoice && <span className="badge badge-green">Chosen: {getTeam(match.matchChoice.chosenTeamId).short}</span>}
                  ...
                  <div className="flex flex-col gap-1.5 min-w-[120px]">
                    {done ? (
                      ...
                      <button className="btn-gold py-1.5 px-3 text-xs" onClick={() => setSelectedMatch(match)}>
                        ⭐ Choose Team
                      </button>
                    ) : contestLike && (match.suggestedResult?.winner || match.suggestedResult?.isDraw) ? (
                      <button
                      ...
                      <>
                        {!isSimulated ? (
                          <button className="btn-success py-1.5 px-3 text-xs" onClick={() => simulate(match)}>
                            🎲 Simulate
                          </button>
                        ) : (
                          <>
                            ...
                            <button className="btn-outline py-1.5 px-3 text-xs" onClick={() => setSimulResult(null)}>
                              Re-simulate
                            </button>
                          </>
                        )}
                        <button className="btn-outline py-1 px-2 text-xs" onClick={() => setSelectedMatch(match)}>
                          Manual
                        </button>
                      </>
                    ) : contestLike && !isLive ? (
                      ...
                      <span className="text-xs text-gray-500 text-center">Choose a team</span>
                    ) : (
        ...
        <Modal
          ...
          title={selectedMatch?.bothSameUser && !selectedMatch?.matchChoice ? 'Choose Your Team' : 'Enter Match Result'}
        >
          {selectedMatch && (
            <div>
              {selectedMatch.bothSameUser && !selectedMatch.matchChoice ? (
                <>
                  <Alert type="info" className="mb-4">
                    You own both teams in this match. Choose one side now. The other team will count as the opponent for this match.
                  </Alert>
                  <div className="flex items-center justify-around mb-6">
                    <div className="text-center">
                      ...
                      <button
                        ...
                        onClick={() => handleChooseTeam(selectedMatch, selectedMatch.team1)}
                        disabled={!!choosing}
                      >
                        {choosing === selectedMatch.id ? <Spinner size="sm" /> : 'Choose This Team'}
                      </button>
                    </div>
                    <div className="text-center">
                    ...
                    <div className="text-center">
                      ...
                      <button
                        className="btn-gold py-2 px-5 text-sm mt-3"
                        onClick={() => handleChooseTeam(selectedMatch, selectedMatch.team2)}
                        disabled={!!choosing}
                      >
                        {choosing === selectedMatch.id ? <Spinner size="sm" /> : 'Choose This Team'}
                      </button>
                  ...
                  <Alert type="success">Once you choose, the other team will be treated as the opponent for this match.</Alert>
                </>
              ) : (
                <>
                  <div className="flex items-center justify-around mb-6">
                  ...
                  <Alert type="info">Winner gets +2 pts · Draw gives +1 pt each · Loser gets 0</Alert>
                </>
              )}
            </div>
          )}
